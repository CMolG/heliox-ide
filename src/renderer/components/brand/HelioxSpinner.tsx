/**
 * HelioxSpinner.tsx — Heliox loading mark
 *
 * Responsibility:
 * - Renders the Heliox diamond (the logo's inner rhombus, lifted out of its square
...
export function HelioxSpinner({ size = 28, spinning = true, speed = 1.8, className = '' }: HelioxSpinnerProps) {
  const inner = Math.round(size * 0.62);
  return (
    <div
      className={`heliox-spinner ${className}`.trim()}
      style={{
        width: size,
        height: size,
        display: 'flex',
