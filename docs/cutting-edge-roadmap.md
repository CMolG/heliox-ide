...
A parity suite proving a flow exported from the TS engine runs **identically** in the Java `HelioxRuntime` (`sdk/java`). For "exportable agentic flows" as THE selling point, this is mandatory, not optional.
...
- ✅ **Phase 4 — DONE & validated live.**
  ...
  - Configurable judge model via `HELIOX_JUDGE_MODEL` (judge ≠ agent) to reduce self-judging bias.
