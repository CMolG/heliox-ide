...
1. **Default deny on every PROTECTED entry point.** Every route that serves authenticated users or private data, and every API endpoint and state-changing server action, must begin with an explicit auth check. The absence of an auth guard on a protected resource is a bug, not a choice. Middleware-level protection does not substitute for per-handler checks on sensitive operations. (Public surfaces are the deliberate exception — see rule 8.)
...
8. **Public surfaces are public by design — never guard them.** Marketing/landing pages, the authentication pages themselves (login, signup, password-reset, email-verification), and legal/status pages must remain reachable WITHOUT authentication — never place them behind the route guard. Default-deny targets authenticated app/dashboard/account routes, private data, and every state-changing endpoint, not the public shell. Redirecting an anonymous visitor or a search crawler away from a public marketing route such as `/` to `/login` is itself a defect, not enforcement.
